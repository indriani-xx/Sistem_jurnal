...
<body>
    <div class="container">
        ...
        <div class="page-header">
            <h2><i class="fas fa-book"></i>Data Kelas</h2>
            <p>Kelola Data Kelas SMK Negeri 1 Sukawati</p>
        </div>
        ...
        $total_data = mysqli_fetch_assoc($total_kelas);
        ...
        if (isset($_GET['cari']) && $_GET['cari'] != "") {
            ...
            $result = mysqli_query($koneksi, "SELECT * FROM kelas, guru, jurusan  WHERE kelas.id_guru=guru.id_guru
                                             AND kelas.id_jurusan=jurusan.id_jurusan ORDER BY id_kelas DESC");
        }
        ?>
        ...
        <div class="content-container">
            ...
            <div class="action-bar">
                <form class="search-form" method="get" action="">
                    ...
                    <input type="search" name="cari" placeholder="Cari Jurnal..." value="<?= htmlspecialchars($cari) ?>">
                    <button type="submit"><i class="fas fa-search"></i></button>
                </form>
                <a href="kelas_tambah.php" class="btn-primary">
                    <i class="fas fa-plus"></i>Input Kelas
                </a>
            </div>
                        ...
                        while ($row = mysqli_fetch_assoc($result)) { ?>
                            <tr>
                                <td data-label="No"><?= $no++ ?></td>
                                 <td data-label="Nama Kelas"><?= $row['nama_kelas'] ?></td>
                                <td data-label="Guru"><?= $row['nama_guru'] ?></td>
                                <td data-label="Jurusan"><?= $row['nama_jurusan'] ?></td>
                                
                                <td data-label="Aksi">
                                    <div class="btn-group">
                                        <a href="kelas_edit.php?id=<?= $row['id_kelas'] ?>" class="btn-warning">
